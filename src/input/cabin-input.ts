...
/**
 * The free camera's controls, read from the same keys but kept out of
 * `PlayerCommand`. Spectating is a local view state: it must never reach the
 * simulation, and it must never be replicated to the other player.
 */
export interface SpectatorRead {
  move: { x: number; y: number };
  vertical: number;
  sprint: boolean;
  /** True on the frame V was pressed. */
  toggle: boolean;
}
...
export class CabinInputController {
  ...
  /**
   * The free camera's controls. Read every frame whether or not the camera is
   * detached, because the toggle itself lives here — and it is consumed, so
   * holding V does not strobe in and out of spectator mode.
   */
  public readSpectator(): SpectatorRead {
    if (!this.active) return { move: { x: 0, y: 0 }, vertical: 0, sprint: false, toggle: false };
    const pad = navigator.getGamepads().find((entry) => entry?.connected);
    const axisX = deadZone(pad?.axes[0] ?? 0);
    const axisY = deadZone(pad?.axes[1] ?? 0);
    const keyboard = {
    ...
    return {
      move: Math.abs(axisX) + Math.abs(axisY) > 0 ? { x: axisX, y: axisY } : keyboard,
      vertical:
...
function isGameKey(code: string): boolean {
  // Space and V are the free camera's: rise, and detach. They are claimed here
  // so the browser does not scroll the page out from under a spectating player.
  return /^(Space|Digit[123]|Key[ABCDEFGHIJLQRSVW]|Arrow(Up|Down|Left|Right)|Shift(Left|Right)|Control(Left|Right))$/.test(
    code,
  );
}
