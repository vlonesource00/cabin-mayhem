...
/**
 * Turn the stick into playfield motion for the crew member standing `where`.
 *
 * `cameraRelativeMovement` assumes the playfield and the world are the same
 * frame, which holds on every deck of the ship and on none of the stairs: a
 * tower's playfield is its flights unrolled, so forward on the playfield
 * reverses in the world twice per deck. Subtracting the local heading cancels
 * that, and forward becomes forward everywhere.
 *
 * `look` gets the same correction. It is the crew member's facing as the
 * simulation sees it — what a thrown crate flies along, what an interaction
 * reaches towards — and the simulation measures those on the playfield.
 */
export function playfieldRelativeMovement(
  move: Vec2,
  yaw: number,
  where: PlayerState | undefined,
): { move: Vec2; look: Vec2 } {
  const compartment = where ? compartmentById(where.compartmentId) : undefined;
  const heading = compartment ? playfieldHeading(compartment, where!.position) : Math.PI;
  return cameraRelativeMovement(move, yaw - (heading - Math.PI));
}
...
export class FirstPersonController {
  ...
  public transform(command: PlayerCommand, where?: PlayerState): PlayerCommand {
    const relative = playfieldRelativeMovement(command.move, this.yaw, where);
  ...
  /**
   * Face the way a doorway leaves you facing. Called on the frame the crew
  ...
  public updateCamera(
    ...
    // The camera streams from the compartment it stands in, so the local frame
    // and the render origin are the same room. On a stair tower `position.y`
    // carries the climb.
    const position = cabinToWorld(player.position, 0, player.compartmentId, player.compartmentId);
  ...
  /** Look angles, shared with the free camera so toggling never snaps the view. */
  public lookAngles(): { yaw: number; pitch: number } {
