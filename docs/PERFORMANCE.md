...
| Ship exterior GLB                 | ≤ 8 MB, ≤ 96 draw meshes                             | `validate:assets`                          |
...
| Tier | Condition                                | Cost                                                                          |
| ---- | ---------------------------------------- | ----------------------------------------------------------------------------- |
| X0   | Crew on an exterior deck                 | Whole asset, shadows on: hull, superstructure, funnels, boats, rails, signage |
...
X1 is selected by mesh name, the same way `CompartmentStreamer.applyDetail`
already picks a compartment's shell out of its dressing, so the tier costs a
visibility flag per mesh and no extra draw state.
